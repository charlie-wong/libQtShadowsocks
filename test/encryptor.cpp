...
void Encryptor::testAesGcm()
{
    ...
    QCOMPARE(encrypted.length(),
        cInfo.saltLen + 2 + cInfo.tagLen + testData.length() + cInfo.tagLen
    );
    ...
    QCOMPARE(encrypted.length(),
        2 + cInfo.tagLen + testData.length() + cInfo.tagLen
    );
...
void Encryptor::testAesGcmMultiChunks()
{
    ...
    encrypted += encryptor.encrypt(std::string(" Bye"));
    std::string decrypted = decryptor.decrypt(encrypted);
