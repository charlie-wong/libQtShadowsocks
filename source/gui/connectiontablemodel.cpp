...
QVariant ConnectionTableModel::headerData(int section,
...
QModelIndex ConnectionTableModel::index(int row, int column,
    const QModelIndex &) const
{
    if(row < 0 || row >= items.size()) {
        ...
        // column is ignored (all columns have the same effect)
        return createIndex(row, column, item);
    }
}

bool ConnectionTableModel::removeRows(int row, int count,
...
bool ConnectionTableModel::appendConnection(Connection *con,
    const QModelIndex &parent)
{
    ConnectionItem *newItem = new ConnectionItem(con, this);

    connect(newItem, &ConnectionItem::message,
        this, &ConnectionTableModel::message
    );
    connect(newItem, &ConnectionItem::stateChanged,
        this, &ConnectionTableModel::onConnectionStateChanged
    );
    connect(newItem, &ConnectionItem::latencyChanged,
        this, &ConnectionTableModel::onConnectionLatencyChanged
    );
    connect(newItem, &ConnectionItem::dataUsageChanged,
        this, &ConnectionTableModel::onConnectionDataUsageChanged
    );
...
void ConnectionTableModel::disconnectConnectionsAt(const QString &addr,
    quint16 port)
{
