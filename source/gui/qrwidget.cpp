...
void QRWidget::setQRData(const QByteArray &data)
{
    ...
    QRcode *qrcode =
        QRcode_encodeString(data.constData(), 1, QR_ECLEVEL_L, QR_MODE_8, 1);
...
void QRWidget::paintEvent(QPaintEvent *e)
{
    ...
    painter.translate((this->width() - nSize.width()) / 2,
        (this->height() - nSize.height()) / 2
    );
    painter.scale(nSize.width() / qrImage.width(),
        nSize.height() / qrImage.height()
    );
