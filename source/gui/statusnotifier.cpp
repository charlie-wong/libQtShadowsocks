    ...
    QObject(parent), window(w)
{
    ...
    minimiseRestoreAction =
        new QAction(startHiden ? tr("Restore") : tr("Minimise"), this);
    connect(minimiseRestoreAction, &QAction::triggered,
        this, &StatusNotifier::activate
    );

    systrayMenu.addAction(minimiseRestoreAction);

    systrayMenu.addAction(QIcon::fromTheme(
        "application-exit", QIcon::fromTheme("exit")),
...
void StatusNotifier::showNotification(const QString &msg)
{
    ...
    args << QCoreApplication::applicationName()
         << quint32(0) << "ShadowSocksQt(GUI)" << msg
