    ...
    , instanceRunning(false)
{
    ...
    setupActionIcon();
    notifier =
        new StatusNotifier(this, configHelper->isHideWindowOnStartup(), this);
    ...
    connect(ui->actionShare, &QAction::triggered,
        this, &MainWindow::onShare
    );
    ...
    ui->connectionView->horizontalHeader()->restoreGeometry(
        configHelper->getTableGeometry()
    );
    ui->connectionView->horizontalHeader()->restoreState(
        configHelper->getTableState()
    );
}
...
MainWindow::~MainWindow()
{
    ...
    // while still needed for the functions written above
    delete ui;
}

const QUrl MainWindow::issueUrl =
    QUrl("https://github.com/charlie-wong/ShadowSocksQt/issues");
...
void MainWindow::onAddScreenQRCode()
{
    ...
    if(uri.isNull()) {
        QMessageBox::critical(this, tr("QR Code Not Found"),
...
void MainWindow::onAddScreenQRCodeCapturer()
{
    ...
    connect(capturer, &QRCodeCapturer::qrCodeFound,
        this, &MainWindow::onQRCodeCapturerResultFound, Qt::DirectConnection
    );
    capturer->show();
}
...
{
    QString qrFile =
        QFileDialog::getOpenFileName(this, tr("Open QR Code Image File"),
            QString(), "Images (*.png *jpg *jpeg *xpm)"
        );
...
void MainWindow::onAddFromURI()
{
    ...
    connect(inputDlg, &URIInputDialog::acceptedURI,
    [&](const QString & uri) {
        Connection *newCon = new Connection(uri, this);
        newProfile(newCon);
    });
...
{
    QString file = QFileDialog::getOpenFileName(this,
        tr("Open config.json"), QString(), "JSON (*.json)"
    );
...
{
    QByteArray uri = model->getItem(proxyModel->mapToSource(
        ui->connectionView->currentIndex()).row())->getConnection()->getURI();
...
{
    model->getItem(
        proxyModel->mapToSource(ui->connectionView->currentIndex()).row()
    )->testLatency();
}
...
void MainWindow::onMoveUp()
{
    ...
    int targetRow = proxyModel->mapToSource(
        proxyModel->index(proxyIndex.row() - 1,
            proxyIndex.column(), proxyIndex.parent())
        ).row();
...
void MainWindow::checkCurrentIndex(const QModelIndex &_index)
{
    ...
    ui->actionMoveDown->setEnabled(
        valid ? _index.row() < model->rowCount() - 1 : false
    );
