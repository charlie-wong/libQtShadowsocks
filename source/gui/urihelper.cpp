...
QString URIHelper::decodeImage(const QImage &img)
{
    ...
    zbar::Image image(gimg.bytesPerLine(), gimg.height(),
        "Y800", gimg.bits(), gimg.byteCount()
    );

    scanner.scan(image);
    zbar::SymbolSet res_set = scanner.get_results();

    for(zbar::SymbolIterator it = res_set.symbol_begin();
        it != res_set.symbol_end(); ++it) {
        if(it->get_type() == zbar::ZBAR_QRCODE) {
            // uri will be overwritten if the result is valid
            // this means always the last uri gets used therefore,
            // please only leave one QR code for the sake of accuracy
