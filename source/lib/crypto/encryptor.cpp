...
void Encryptor::initDecipher(const char *data, size_t length, size_t *offset)
{
    ...
    if(cipherInfo.type == Cipher::CipherType::AEAD) {
        ...
        key = Cipher::deriveAeadSubkey(cipherInfo.keyLen, masterKey,
            std::string(data, cipherInfo.saltLen)
        );
    ...
    if(cipherInfo.type == Cipher::CipherType::AEAD) {
        ...
        if(incompleteLength != 0u) {
            ...
            if(dataEnd - data < AEAD_CHUNK_SIZE_LEN + cipherInfo.tagLen) {
                ...
                incompleteChunk = std::string(
                    reinterpret_cast<const char *>(data), dataEnd - data
                );
                return std::string();
            }

            std::string decLength = deCipher->update(data,
                AEAD_CHUNK_SIZE_LEN + cipherInfo.tagLen
            );
